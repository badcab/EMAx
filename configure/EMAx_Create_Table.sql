...
CREATE TABLE IF NOT EXISTS `EMAx_RoomLocation` (
  ...
  `costBaseNonProfit` DECIMAL(6,2) UNSIGNED NOT NULL DEFAULT 0.00,
  `costBaseForProfit` DECIMAL(6,2) UNSIGNED NOT NULL DEFAULT 0.00,
  `costExtraLongNonProfit` DECIMAL(6,2) UNSIGNED NOT NULL DEFAULT 0.00,
  `costExtraLongForProfit` DECIMAL(6,2) UNSIGNED NOT NULL DEFAULT 0.00,
  PRIMARY KEY (`ID`),
  UNIQUE (`name`)
) ENGINE=InnoDB  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
...
CREATE TABLE IF NOT EXISTS `EMAx_PublicProgramMoney` (
  `EMAx_Event_ID` int(11) NOT NULL,
  `revenue` DECIMAL(6,2) UNSIGNED NOT NULL DEFAULT 0.00,
  `expenses` DECIMAL(6,2) UNSIGNED NOT NULL DEFAULT 0.00,
  FOREIGN KEY (EMAx_Event_ID) REFERENCES EMAx_Event (ID)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ;

CREATE TABLE IF NOT EXISTS `EMAx_CONFIG` (
