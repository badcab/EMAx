...
class EventModel
{
	...
	function __construct($id = NULL)
	{
		...
		if($currentDBvalues)
		{
			...
			$option = array();
				...
				$gradeSQL = "SELECT * FROM `EMAx_GradeEventMap` WHERE `EMAx_GradeEventMap`.`EMAx_Event_ID` = '" . $id . "'";
				...
				$optionSQL = "SELECT * FROM `EMAx_OptionEventMap` WHERE `EMAx_OptionEventMap`.`EMAx_Event_ID` = '" . $id . "'";
	...
	public function writeData()
	{
		...
		if($this->ClassObjectArg['ID'])
		{
			...
			$sql = "
				UPDATE `EMAx_Event` SET `EMAx_Organization_ID`='". $this->ClassObjectArg['Organization']  
				."',`EMAx_Person_ID`='". $this->ClassObjectArg['Person']  
				."',`EMAx_RoomLocation_ID`='". $this->ClassObjectArg['RoomLocation']  
				."',`EMAx_Login_ID`='". $this->ClassObjectArg['Login']  
				."',`startTime`='". $this->ClassObjectArg['startTime']  
				."',`endTime`='". $this->ClassObjectArg['endTime']  
				."',`attendance`='". $this->ClassObjectArg['attendance']  
				."',`havingLunch`='". $this->ClassObjectArg['havingLunch']  
				."',`googlURI`='". $googleURI  
				."',`hasPaid`='". $this->ClassObjectArg['hasPaid']  
		...
		else
		{
			...
			$sql = 
				"INSERT INTO `EMAx_Event`(
					...
					'". $this->ClassObjectArg['Person'] ."',
					'". $this->ClassObjectArg['RoomLocation'] ."',
					'". $this->ClassObjectArg['Login'] ."',
					'". $this->ClassObjectArg['startTime'] ."',
					'". $this->ClassObjectArg['endTime'] ."',
					'". $this->ClassObjectArg['attendance'] ."',
					'". $this->ClassObjectArg['havingLunch'] ."',
					'". $googleURI ."',
					'". $this->ClassObjectArg['hasPaid'] ."',
...
//		$connection->exec("DELETE FROM `EMAx_OptionEventMap` WHERE `EMAx_Event_ID` = '". $this->getID() ."'");
//		$connection->exec("DELETE FROM `EMAx_GradeEventMap` WHERE `EMAx_Event_ID` = '". $this->getID() ."'");
		$connection->exec("DELETE FROM `EMAx_Event` WHERE `ID`='" . $this->getID() . "'");
	}	
